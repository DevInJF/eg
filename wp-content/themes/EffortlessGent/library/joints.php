...
function joints_scripts_and_styles() {
  ...
  if (!is_admin()) {
    ...
    // wp_enqueue_script( 'classie', get_template_directory_uri() . '/library/js/classie.js', array( 'jquery' ), '', true );
    // wp_enqueue_script( 'AnimOnScroll', get_template_directory_uri() . '/library/js/AnimOnScroll.js', array( 'jquery' ), '', true );
...
function joints_theme_support() {
	...
	add_image_size('blog-feature-large', 2400, 600, true);		// Featured Large
    
	add_image_size( 'feature-email', 600, 300, true );			// Featured image for email

	add_image_size('blog-thumbnail-small', 640, 320, true);		// Image for small screens
	add_image_size('blog-thumbnail-medium', 1024, 350, true);	// Image for medium up
...
function joints_wpsearch($form) {
	...
	<input type="text" value="' . get_search_query() . '" name="s" id="s" placeholder="'.esc_attr__('What are you looking for?','jointstheme').'" />
...
return '...  <a class="excerpt-read-more" href="'. get_permalink($post->ID) . '" title="'. __('Read ', 'jointstheme') . get_the_title($post->ID).'">'. __('KEEP READING &raquo;', 'jointstheme') .'</a>';
}

// Make sure we have the blog title on the homepage
